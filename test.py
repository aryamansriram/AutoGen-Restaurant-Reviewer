...
def contains_num_with_tolerance(
    text: str, pattern: float, tolerance: float = 0
) -> bool:
...
def public_tests():
    ...
    for i, content in enumerate(contents):
        if not contains_num_with_tolerance(
            content, query_results[i], tolerance=tolerances[i]
        ):
            print(
                TerminalColors.RED + f"Test {i+1} Failed." + TerminalColors.RESET,
                "Expected: ",
                query_results[i],
                "Query: ",
                queries[i],
            ...
            print(
                TerminalColors.GREEN + f"Test {i+1} Passed." + TerminalColors.RESET,
                "Expected: ",
                query_results[i],
                "Query: ",
                queries[i],
